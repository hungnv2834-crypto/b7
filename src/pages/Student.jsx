...
const Student = () => {
  ...
  const fetchData = async (page = 1, limit = 10) => {
    setLoading(true);
    try {
      const response = await axios.get(`http://103.166.183.82:4040/api/v1/student/pageable?page=${page}&limit=${limit}`, {
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://103.166.183.82:4040/api/v1/student/${id}`, {
        headers: { Authorization: `Bearer ${localStorage.getItem('access_token')}` }
      });
  ...
  const handleOk = async () => {
    try {
      ...
      if (editingId) {
        await axios.put(`http://103.166.183.82:4040/api/v1/student/${editingId}`, values, {
          headers: { Authorization: `Bearer ${localStorage.getItem('access_token')}` }
        });
  ...
  const columns = [
    { title: 'Mã SV', dataIndex: 'identityCode', key: 'identityCode' },
  ...
  return (
    <div style={{ background: '#fff', padding: 24, borderRadius: 8 }}>
      ...
      <Modal 
        title={editingId ? "Sửa sinh viên" : "Thêm mới sinh viên"} 
        open={isModalVisible} 
        onOk={handleOk} 
